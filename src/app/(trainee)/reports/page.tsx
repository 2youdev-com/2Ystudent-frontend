...
export default function ReportsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Example: https://2ystudy-backend-xxx.run.app/api
  ...
  }, [token, currentPage, scenarioFilter, language]);
  ...
  // Map legacy sales skill names to engineering equivalents
  const normalizeSkillName = (name: string): string => {
    const skillNameMap: Record<string, string> = {
      ...
      'Relationship Building': 'Safety Awareness',
      'Product Knowledge': 'Practical Application',
      'Closing Technique': 'Critical Thinking',
      // Also handle camelCase keys
      'communication': 'Technical Communication',
      'negotiation': 'Knowledge Depth',
    ...
    return skillNameMap[name] || name;
  };

  const formatScenarioType = (type: string) => {
    const scenarioMap: Record<string, string> = {
      // New engineering keys
      'hvac_systems': t.simulations.scenarios.hvacSystems,
      'electrical_systems': t.simulations.scenarios.electricalSystems,
      'safety_compliance': t.simulations.scenarios.safetyCompliance,
      'plc_automation': t.simulations.scenarios.plcAutomation,
      'industrial_maintenance': t.simulations.scenarios.industrialMaintenance,
      'advanced_troubleshooting': t.simulations.scenarios.advancedTroubleshooting,
      'motor_controls': t.simulations.scenarios.motorControls,
      'bms_systems': t.simulations.scenarios.bmsSystems,
      // Legacy keys (fallback for old session data)
      ...
      <div id="reports-content">
                  ...
                  <div key={skill.skillKey} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-foreground">{normalizeSkillName(skill.skill)}</span>
                      </div>
                ...
                {skills?.skills.every(s => s.averageScore === null) && dashboard && dashboard.completedSessions > 0 && (
                  <div className="text-center py-4 border-t border-border mt-4">
                    <p className="text-sm text-muted-foreground mb-3">
                      {'You have sessions without detailed analysis'}
                    </p>
                    <Button
                      variant="outline"
                      size="sm"
        ...
        {skills?.skills && skills.skills.some(s => s.averageScore !== null) && (
          <Card className="card-hover mb-8">
            ...
            <CardContent>
              <SkillRadarChart
                skills={skills.skills.map(s => ({ ...s, skill: normalizeSkillName(s.skill) }))}
                height={300}
                showBenchmark={true}
              />
            </CardContent>
              ...
              <div className="flex items-center gap-3">
                <Select value={scenarioFilter} onValueChange={setScenarioFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="hvac_systems">{t.simulations.scenarios.hvacSystems}</SelectItem>
                    <SelectItem value="electrical_systems">{t.simulations.scenarios.electricalSystems}</SelectItem>
                    <SelectItem value="safety_compliance">{t.simulations.scenarios.safetyCompliance}</SelectItem>
                    <SelectItem value="plc_automation">{t.simulations.scenarios.plcAutomation}</SelectItem>
                    <SelectItem value="industrial_maintenance">{t.simulations.scenarios.industrialMaintenance}</SelectItem>
                    <SelectItem value="advanced_troubleshooting">{t.simulations.scenarios.advancedTroubleshooting}</SelectItem>
                    <SelectItem value="motor_controls">{t.simulations.scenarios.motorControls}</SelectItem>
                    <SelectItem value="bms_systems">{t.simulations.scenarios.bmsSystems}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
        ...
        {/* Voice Learning Reports Section */}
        <div className="mt-8">
          <VoiceReportsSection />
        </div>
      </div>
