...
import { studentCoursesApi, Course, Lesson } from '@/lib/api/trainee-courses.api';
...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await studentCoursesApi.getCourse(courseId);
  ...
  const handleLessonComplete = async () => {
    if (selectedLesson && !completedLessons.has(selectedLesson.id)) {
      ...
      try {
        ...
        // Save to backend
        await studentApi.completeLecture(selectedLesson.id, timeSpentMinutes);
  ...
  const getVideoEmbedUrl = (lesson: Lesson) => {
    const videoId = studentCoursesApi.getYouTubeVideoId(lesson.videoId || lesson.videoUrl);
    if (videoId) {
  ...
  const objectives = studentCoursesApi.getCourseObjectives(course, isRTL);
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Badge className={cn("border", studentCoursesApi.getDifficultyColor(course.difficulty))}>
                  {studentCoursesApi.getDifficultyLabel(course.difficulty, isRTL)}
                </Badge>
                <Badge variant="outline">{studentCoursesApi.getCategoryLabel(course.category, isRTL)}</Badge>
              </div>
              ...
              <div className="aspect-video bg-black">
                {selectedLesson && (
                  <iframe
                    ...
                    title={studentCoursesApi.getLessonTitle(selectedLesson, isRTL)}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <h2 className="text-xl font-semibold text-foreground mb-2 leading-relaxed">
                        {studentCoursesApi.getLessonTitle(selectedLesson, isRTL)}
                      </h2>
                      <p className="text-muted-foreground leading-relaxed">
                        {studentCoursesApi.getLessonDescription(selectedLesson, isRTL)}
                      </p>
                    </div>
                    <Button
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <Card className="border-2 border-[#0089B8]/30 bg-gradient-to-br from-[#0089B8]/5 via-[#0089B8]/5 to-[#0089B8]/5 overflow-hidden">
              <CardContent className="p-4">
                  ...
                  <div className="p-4 bg-gradient-to-br from-[#0089B8]/10 to-[#0089B8]/10 rounded-xl border border-[#0089B8]/20 mb-3 relative overflow-hidden">
                    ...
                    <div className="relative">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-bold text-[#0089B8]">
                          Study with AI Teacher
                        </span>
                      </div>
                      <p className="text-sm text-muted-foreground mb-3 leading-relaxed">
                        {`Get personalized explanation of "${studentCoursesApi.getLessonTitle(selectedLesson, isRTL)}" with instant quizzes.`}
                      </p>
                ...
                {/* AI Practice Section */}
                <div className="p-4 bg-gradient-to-br from-primary/5 to-cyan-500/5 rounded-xl border border-primary/20 mb-3">
                  ...
                  <div className="flex gap-2">
                    <Link href={`/voice-training?courseId=${courseId}`} className="flex-1">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={cn(
                                ...
                                {studentCoursesApi.getLessonTitle(lesson, isRTL)}
                              </p>
