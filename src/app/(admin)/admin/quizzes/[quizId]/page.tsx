...
export default function EditQuizPage() {
  ...
  const handleSave = async () => {
    ...
    if (questions.some((q) => !q.questionText.trim())) { setError('All questions must have text'); return; }
    if (questions.some((q) => !q.options.some((o) => o.isCorrect))) { setError('Each question must have a correct answer'); return; }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <CardContent className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <Input value={title} onChange={(e) => { setTitle(e.target.value); setTitleAr(e.target.value); }} />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700">{t.quiz.quizDescription}</label>
              <Input value={description} onChange={(e) => { setDescription(e.target.value); setDescriptionAr(e.target.value); }} />
            </div>
          </div>
          ...
          <Card key={qIdx} className="border border-gray-200 dark:border-gray-700">
            <CardContent className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                <Input value={q.questionText} onChange={(e) => updateQuestion(qIdx, { questionText: e.target.value, questionTextAr: e.target.value })} placeholder="Question text" />
              </div>
              <div className="space-y-2">
                {q.options.map((opt, oIdx) => (
                  <div key={oIdx} className="flex items-center gap-2">
                    ...
                    {/* TEMPORARY: Arabic fields hidden — English only mode */}
                    <Input value={opt.optionText} onChange={(e) => updateOption(qIdx, oIdx, { optionText: e.target.value, optionTextAr: e.target.value })} placeholder={`Option ${oIdx + 1}`} className={cn('flex-1', opt.isCorrect && 'border-green-300 bg-green-50')} />
              ...
              {/* TEMPORARY: Arabic fields hidden — English only mode */}
              <div className="grid grid-cols-1 gap-3">
                <Input value={q.explanation || ''} onChange={(e) => updateQuestion(qIdx, { explanation: e.target.value, explanationAr: e.target.value })} placeholder="Explanation (optional)" />
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="flex justify-end gap-3 pt-4 border-t">
        ...
        <Button onClick={handleSave} disabled={saving} className="bg-[#0089B8] hover:bg-[#0089B8]/90">
