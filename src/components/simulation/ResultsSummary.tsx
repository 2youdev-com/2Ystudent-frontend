...
const gradeConfig = {
  A: { bg: 'bg-gradient-to-br from-blue-500 to-green-600', text: 'text-white', glow: 'shadow-blue-500/30', label: 'Excellent' },
  B: { bg: 'bg-gradient-to-br from-[#0089B8] to-[#007AA6]', text: 'text-white', glow: 'shadow-[#0089B8]/30', label: 'Great' },
...
// Engineering skill name mappings
...
function SkillScore({ name, detail, index }: { name: string; detail: SkillScoreDetail; index: number }) {
  // Safety: handle undefined or malformed detail from backend
  const safeDetail = detail || { score: 0, benchmark: 75, trend: 'stable' as const, evidence: [], tips: [] };
  const score = typeof safeDetail.score === 'number' ? safeDetail.score : 0;
  ...
  return (
    <div
      className={cn(
        ...
        isStrength && "bg-blue-50/50 border-blue-200/60",
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          {isStrength && <CheckCircle2 className="h-4 w-4 text-blue-500" />}
      ...
      <div className="relative h-2.5 bg-slate-200 rounded-full overflow-hidden">
        <div
          className={cn("absolute inset-y-0 left-0 rounded-full transition-all duration-1000 ease-out", getScoreColor(score))}
          style={{ width: `${score}%` }}
        />
        {/* Benchmark indicator */}
        <div
          className="absolute top-0 h-full w-0.5 bg-slate-400"
          style={{ left: `${safeDetail.benchmark || 75}%` }}
        />
      </div>

      {safeDetail.tips && safeDetail.tips.length > 0 && (
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      await generateTextReportPDF({
        ...
        mentorProfile: sessionData.mentorProfile,
        ...
        analysis: {
          ...
          skillScores: {
            technicalCommunication: { score: skillScores.technicalCommunication?.score ?? 0, tips: skillScores.technicalCommunication?.tips || [] },
            knowledgeDepth: { score: skillScores.knowledgeDepth?.score ?? 0, tips: skillScores.knowledgeDepth?.tips || [] },
            problemSolving: { score: skillScores.problemSolving?.score ?? 0, tips: skillScores.problemSolving?.tips || [] },
            safetyAwareness: { score: skillScores.safetyAwareness?.score ?? 0, tips: skillScores.safetyAwareness?.tips || [] },
            practicalApplication: { score: skillScores.practicalApplication?.score ?? 0, tips: skillScores.practicalApplication?.tips || [] },
            criticalThinking: { score: skillScores.criticalThinking?.score ?? 0, tips: skillScores.criticalThinking?.tips || [] },
          },
          ...
          conversationMetrics: {
            talkTimeRatio: conversationAnalysis?.talkTimeRatio ?? 0,
            questionsAsked: conversationAnalysis?.questionAsked ?? 0,
            technicalAccuracy: conversationAnalysis?.technicalAccuracy ?? 0,
            depthOfReasoning: conversationAnalysis?.depthOfReasoning ?? 0,
          },
        },
      });
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto space-y-6 p-4">
        ...
        <CardHeader className="pb-4">
          <CardTitle className="flex items-center gap-3 text-slate-800">
            ...
            <span className="ml-auto text-xs font-normal text-slate-400">
              | = Engineering Benchmark (75)
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid md:grid-cols-2 gap-4">
            <SkillScore name="Technical Communication" detail={skillScores.technicalCommunication} index={0} />
            <SkillScore name="Knowledge Depth" detail={skillScores.knowledgeDepth} index={1} />
            <SkillScore name="Problem Solving" detail={skillScores.problemSolving} index={2} />
            <SkillScore name="Safety Awareness" detail={skillScores.safetyAwareness} index={3} />
            <SkillScore name="Practical Application" detail={skillScores.practicalApplication} index={4} />
            <SkillScore name="Critical Thinking" detail={skillScores.criticalThinking} index={5} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-slate-200/60 shadow-lg">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <StatCard
              value={`${Math.round((conversationAnalysis?.talkTimeRatio ?? 0) * 100)}%`}
              label="Your Talk Time"
              icon={MessageSquare}
            />
            <StatCard
              value={conversationAnalysis?.questionAsked ?? 0}
              label="Questions Asked"
              icon={Lightbulb}
            />
            <StatCard
              value={conversationAnalysis?.technicalAccuracy ?? 0}
              label="Technical Accuracy"
              icon={Trophy}
            />
            <StatCard
              value={conversationAnalysis?.depthOfReasoning ?? 0}
              label="Depth of Reasoning"
              icon={CheckCircle2}
            />
            <StatCard
              value={`${(conversationAnalysis?.averageResponseTime ?? 0).toFixed(0)}s`}
              label="Avg Response"
              icon={Target}
            />
          </div>
      ...
      <div className={cn(
        ...
        <Button
          ...
          size="lg"
          className="flex-1 bg-[#0089B8] hover:bg-[#007AA6] text-white font-semibold shadow-lg shadow-[#0089B8]/25 transition-all duration-300"
        >
          View Full Report
        ...
        {sessionData && (
          <Button
            ...
            className="flex-1 sm:flex-none border-2 border-blue-300 hover:border-blue-400 hover:bg-blue-50 text-blue-700 font-semibold transition-all duration-300"
          >
            {isExportingPDF ? (
