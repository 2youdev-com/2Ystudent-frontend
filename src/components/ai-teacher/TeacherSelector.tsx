...
export function TeacherSelector({ onSelect, className, showAllTeachers = false }: TeacherSelectorProps) {
  ...
  const { assignedTeacher: storeAssignedTeacher, activeTeacher, setActiveTeacher, groupAITeacherNames, hasGroupFilter } = useTeacherStore();
  ...
  // If showAllTeachers, show all teachers (or filtered by group assignment)
  const allTeachers = hasGroupFilter
    ? Object.values(TEACHERS).filter((t) => groupAITeacherNames.includes(t.name))
