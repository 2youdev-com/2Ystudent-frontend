...
function getDefaultAnalysis(): ParsedAnalysis {
  return {
    ...
    weaknesses: ['Not analyzed yet'],
    improvements: ['Complete a session to get analysis'],
    summary: 'This session has not been analyzed',
  };
}

export function VoiceReportsSection({ className, studentId }: VoiceReportsSectionProps) {
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      const url = studentId
        ? `${apiUrl}/admin/trainee/${studentId}/voice-sessions`
  ...
  const parseAnalysis = (analysisJson: string | null): ParsedAnalysis => {
    ...
    try {
      ...
      return {
        ...
        summary: parsed.summary || 'This session has not been analyzed',
      };
    } catch {
    ...
    return (
      <Card className={className}>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-gray-500">Loading voice session reports...</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 gap-2">
            <CardTitle className="text-sm font-medium leading-relaxed">Voice Sessions</CardTitle>
            <Phone className="h-5 w-5 text-primary flex-shrink-0" />
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold mt-1">{stats.totalSessions}</div>
            <p className="text-xs text-muted-foreground mt-2 leading-relaxed">Total training sessions</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 gap-2">
            <CardTitle className="text-sm font-medium leading-relaxed">Average Score</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2 leading-relaxed">Across all sessions</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 gap-2">
            <CardTitle className="text-sm font-medium leading-relaxed">Best Score</CardTitle>
            <Award className="h-5 w-5 text-yellow-500 flex-shrink-0" />
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-green-600 mt-1">{safePercent(stats.bestScore)}</div>
            <p className="text-xs text-muted-foreground mt-2 leading-relaxed">Highest score achieved</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 gap-2">
            <CardTitle className="text-sm font-medium leading-relaxed">Learning Time</CardTitle>
            <Clock className="h-5 w-5 text-[#0089B8] flex-shrink-0" />
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold mt-1">
              {Math.floor(safeNumber(stats.totalDuration, 0) / 60)} <span className="text-lg font-normal">min</span>
            </div>
            <p className="text-xs text-muted-foreground mt-2 leading-relaxed">Total training time</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Your voice training performance evaluations
            </CardDescription>
          </div>
          <button
        ...
        <CardContent>
          {sessions.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-sm mt-1">Complete a voice training session to see reports here</p>
            </div>
          ) : (
                    ...
                    {isExpanded && (
                      <div className="border-t bg-gray-50 p-4 space-y-4">
                        ...
                        <div className="bg-gray-100 rounded-lg p-4">
                          <h5 className="font-semibold text-gray-700 mb-2">Performance Summary</h5>
