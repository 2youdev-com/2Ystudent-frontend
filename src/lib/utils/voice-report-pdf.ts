...
/**
 * Voice Learning Report PDF Generator
 *
 * Generates professional English-layout PDFs for voice training session reports.
 * Uses html2canvas fallback for proper text rendering.
 */
...
export async function generateVoiceReportPDF(report: VoiceReportData): Promise<void> {
  ...
  const safeReport = {
    ...
    strengths: safeArray(report.strengths, ['No strengths identified']),
    weaknesses: safeArray(report.weaknesses, ['No weaknesses identified']),
    improvements: safeArray(report.improvements, ['No suggestions available']),
    summary: safeStr(report.summary, 'This session has not been analyzed'),
  };
  ...
  const categories = [
    { label: 'Introduction', score: safeReport.breakdown.opening },
    { label: 'Knowledge Depth', score: safeReport.breakdown.needsDiscovery },
    { label: 'Problem Solving', score: safeReport.breakdown.objectionHandling },
    { label: 'Critical Thinking', score: safeReport.breakdown.persuasion },
    { label: 'Practical Application', score: safeReport.breakdown.closing },
    { label: 'Technical Communication', score: safeReport.breakdown.communication },
  ];
  ...
  const htmlContent = `
    ...
    <body>
      <div class="page">
        <div class="header">
          <h1>Voice Learning Report</h1>
          <p>AI-Assisted Technical Knowledge Assessment</p>
        </div>

        <div class="meta-info">
          <span>📅 Date: ${formatDate(safeReport.startTime)}</span>
          <span>⏱️ Duration: ${durationMins} min</span>
          ${safeReport.studentName ? `<span>👤 Student: ${safeReport.studentName}</span>` : ''}
        </div>
        ...
        <div class="footer">
          Report generated by 2YStudy Learning Platform
        </div>
      </div>
    </body>
    </html>
