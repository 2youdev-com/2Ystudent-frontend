...
export async function exportChatTranscriptToPDF(
  ...
  header.innerHTML = `
    ...
    <p style="color: #64748b; font-size: 14px;">Learning Performance Report</p>
  `;
  ...
  const infoItems = [
    { label: 'Topic', value: data.scenarioType },
    { label: 'Level', value: data.difficultyLevel },
    { label: 'Duration', value: `${Math.round(data.duration / 60)} min` },
    { label: 'Date', value: new Date(data.completedAt).toLocaleDateString('en-GB') },
  ...
  messagesHeader.textContent = 'Conversation';
  ...
  data.messages.forEach(msg => {
    const isStudent = msg.speaker === 'student' || msg.speaker === 'trainee';
    const messageDiv = document.createElement('div');
    messageDiv.style.display = 'flex';
    messageDiv.style.flexDirection = isStudent ? (isRTL ? 'row' : 'row-reverse') : (isRTL ? 'row-reverse' : 'row');
    ...
    avatar.style.backgroundColor = isStudent ? '#dbeafe' : '#f3f4f6';
    avatar.style.display = 'flex';
    avatar.style.alignItems = 'center';
    avatar.style.justifyContent = 'center';
    avatar.style.fontSize = '14px';
    avatar.style.flexShrink = '0';
    avatar.textContent = isStudent ? 'S' : 'M';
    ...
    speakerLabel.style.color = isStudent ? '#2563eb' : '#4b5563';
    speakerLabel.style.marginBottom = '4px';
    speakerLabel.textContent = isStudent
      ? 'You (Student)'
      : 'AI Mentor';

    const bubble = document.createElement('div');
    bubble.style.backgroundColor = isStudent ? '#3b82f6' : '#f3f4f6';
    bubble.style.color = isStudent ? 'white' : '#1e293b';
    bubble.style.padding = '12px 16px';
    bubble.style.borderRadius = '16px';
    bubble.style.borderTopRightRadius = isStudent && !isRTL ? '4px' : '16px';
    bubble.style.borderTopLeftRadius = !isStudent && !isRTL ? '4px' : isStudent && isRTL ? '4px' : '16px';
    ...
    timestamp.textContent = new Date(msg.timestamp).toLocaleTimeString('en-GB');
