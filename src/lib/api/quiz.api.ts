...
export const quizApi = {
  ...
  /** List published quizzes available for student */
  ...
  getStudentHistory: async (): Promise<{ history: StudentAttemptHistoryItem[] }> => {
    return apiClient.get<{ history: StudentAttemptHistoryItem[] }>('/quizzes/history');
  },
  ...
  // ==========================================
  // Quiz Assignment (Admin)
  // ==========================================

  /** Assign quizzes to a group */
  assignQuizzesToGroup: async (groupId: string, quizIds: string[]): Promise<{ added: number; skipped: number }> => {
    return apiClient.post<{ added: number; skipped: number }>(`/groups/${groupId}/quizzes`, { quizIds });
  },

  /** Unassign a quiz from a group */
  unassignQuizFromGroup: async (groupId: string, quizId: string): Promise<void> => {
    return apiClient.delete(`/groups/${groupId}/quizzes/${quizId}`);
  },

  /** Get quizzes assigned to a group */
  getGroupQuizzes: async (groupId: string): Promise<{ quizzes: QuizListItem[] }> => {
    return apiClient.get<{ quizzes: QuizListItem[] }>(`/groups/${groupId}/quizzes`);
  },
};
