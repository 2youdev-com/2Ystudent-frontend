...
export const settingsApi = {
  ...
  // Reset all training data (DANGER - admin only)
  resetData: async (confirmText: string): Promise<{ message: string; affectedStudents: number }> => {
    return apiClient.delete<{ message: string; affectedStudents: number }>('/settings/reset-data', {
      data: { confirmText },
    });
  },
};
